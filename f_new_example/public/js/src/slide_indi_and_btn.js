...
$.ajax({
  async:false, datatype:'json',url : url,success:function(data){return rel =data;}
});
...
indicator.find('li').css({display:'inline-block',
marginLeft:'0.5rem'});
indicator.find('a').css({display:'block',width:'26px',
height:'26px',backgroundColor:'#777', borderRadius:'100%'});
...
btnArea.css({position:'absolute', top:'50%', left:'5%',zIndex:500,
width:'90%', height:0});
...
//li 마지막 요소 복제 첨부 후디자인수정

slideUl.children('li').eq(-1).clone().prependTo(slideUl);
//--------------------------------------------

slideLen++;
  slideUl.css({width:100*slideLen+'%', height:'100%',marginLeft:'-100%',
...
}else{indicator.siblings().css({backgroundColor:indiDefault});
